import { ChangeDetectorRef, Component, NgZone, OnInit, signal, ViewChild } from '@angular/core';
...
import { CreatePedidoDialog } from '../create-pedido-dialog/create-pedido-dialog';
...
export class Pendientes implements OnInit {
  ...
  openCreatePedido(pedido: any): void {
    const dialogRef = this.dialog.open(CreatePedidoDialog, {
      width: '800px',
      maxWidth: '100vw',
      disableClose: true,
      data: pedido
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result && pedido == null) {
        this.dataSource.data = [result.data.result, ...this.dataSource.data]
      } else {
        const index = this.dataSource.data.findIndex(p => p.idOrder === result.data.result.idOrder);
        if (index !== -1) {
          this.dataSource.data[index] = result.data.result;
  ...
  markSend(idOrder: number) {
    this.orderService.markSend(idOrder).subscribe({
      next: (res: any) => {
        if (res) {
          this.dataSource.data = this.dataSource.data.filter(order => order.idOrder != idOrder);
          this.notificationService.showSuccess("Orden marcada como enviada");
        }
      }
    })
  }
  ...
  deletePedido(pedido: any) {
    this.confirmationDialog.confirmDelete("¿Está seguro que desea eliminar el pedido?").subscribe(confirm => {
      if (confirm) {
        this.orderService.deleteOrder(pedido.idOrder).subscribe({
          next: (response: any) => {
            if (response) {
              this.dataSource.data = this.dataSource.data.filter(order => order.idOrder != pedido.idOrder);
              this.notificationService.showSuccess("Orden eliminada correctamente");
            }
          },
          error: (error) => {
            console.log(error);
